          ...
          {commentForum.comments ? (
            <div>
              {commentForum.comments.map((item, index) => (
                <div className={'p-1 my-1 rounded-lg bg-slate-300'}>
                  <p className="text-lg">{item.contenu}</p>
                  <p className="text-sm">{formatDate(item.dateCom)} - {item.heureCom}</p>
                </div>
              ))}
            </div>
          ) : ('')}
            ...
            >
              <div className="flex flex-row justify-between">
                <div className="flex flex-row space-x-2">
                  <li className="text-lg font-bold" onClick={() => setForumComment(item.idForum)}>{item.titre}</li>
              ...
              <textarea
                ...
                cols="10"
                rows="10"
                className={`bg-white w-full mt-2 rounded ${com === item.idForum ? 'visible' : 'hidden'}`}
